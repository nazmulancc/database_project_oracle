...
UPDATE vehicle
 ...
 WHERE cust_no = (
    SELECT cust_no
      FROM customer
     WHERE upper(cust_phone) = upper('6715573197')
);
...
UPDATE service_job
   SET
    sj_task_description = 'Removed material causing jam in belt mechanism'
 WHERE serv_no = (
...
SELECT serv_no,
       ...
       to_char(
           serv_req_pickup,
           'DD-Mon-YYYY HH24:MI'
       ) AS requested_pickup_time,
       to_char(
           serv_ready_pickup,
           'DD-Mon-YYYY HH24:MI'
       ) AS actual_ready_time
  FROM service
 WHERE veh_vin = (
    SELECT veh_vin
      FROM vehicle
